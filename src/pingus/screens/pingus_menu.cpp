  ...
  options_button()
{
  ...
  editor_button = gui_manager->create<MenuButton>(
    this, Vector2i(size_.width/2 + 260,
                   size_.height/2 + 85),
    _("Editor"),
  ...
  Sound::PingusSound::play_music("pingus-1.ogg");
}
...
PingusMenu::draw_background(DrawingContext& gc)
{
  ...
  /*gc.print_left(Fonts::pingus_small, Vector2i(gc.get_width()/2 - 400 + 25, gc.get_height()-140),
                "Pingus Windows - Copyright (C) 1998-2011 Ingo Ruhnke <grumbel@gmail.com>\n"
...
PingusMenu::resize(const Size& size_)
{
  ...
  start_button->set_pos(size.width / 2,
                        size.height / 2 - 20);
  editor_button->set_pos(size.width / 2 + 260,
                         size.height / 2 + 85);

  contrib_button->set_pos(size.width / 2,
                          size.height / 2 + 50);

  options_button->set_pos(size.width/2,
                          size.height / 2 + 120);
