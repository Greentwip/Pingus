...
SEXP_API int list_length(Value const& sx);
SEXP_API Value const& list_ref(Value const& sx, int index);
SEXP_API bool is_list(Value const& sx);
SEXP_API Value const& assoc_ref(Value const& sx, std::string const& key);
